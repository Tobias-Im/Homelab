...
$SourcePath = Join-Path (Split-Path -Parent $PSScriptRoot) "Proxmox"

# We create another variable named $RemoteDir for the destination on the Linux server.
$RemoteDir = "~/Scripts/Proxmox"
...
Write-Host "Proxmox Script Deployer" -ForegroundColor Cyan
...
$Username = Read-Host "Enter SSH Username (e.g. root)"
...
# FIX APPLIED HERE: We used ${ServerIP} to separate the name from the colon.
scp -r "$SourcePath\*" "${Username}@${ServerIP}:$RemoteDir/"
...
# NO READ-HOST HERE. The script will finish and close immediately.